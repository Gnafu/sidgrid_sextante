...
public class PolygonToUnsaturatedZoneAlgorithm extends GeoAlgorithm{
	...
	public void defineCharacteristics() {
		setName(Sextante.getText("Polygon to UZF Zone"));
		...
		try {
			...
			addOutputVectorLayer(RESULT, Sextante.getText("UZF layer zone"), OutputVectorLayer.SHAPE_TYPE_POLYGON);
	...
	public boolean processAlgorithm() throws GeoAlgorithmExecutionException {
		...
		final Class[] outputFieldTypes = new Class[3 + (dStressPeriod*4)];
	    final String[] outputFieldNames = new String[3 + (dStressPeriod*4)];
	    String riga = "row";
	    String colonna = "col";
	    String bound = "uzfbound";
	    String nome = "UZF_unsaturated";
	    ...
	    int n = 1;
	    int rec = count;
	    for (int s = count; s < 3 + dStressPeriod; s++)
	    {
	    	outputFieldNames[rec] =  "sp_"+(n) + "_finf";
	    	outputFieldNames[rec+1] =  "sp_"+(n)+ "_pet";
	    	outputFieldNames[rec+2] =  "sp_"+(n)+ "_extdp";
	    	outputFieldNames[rec+3] =  "sp_"+(n)+ "_extwc";
	    	n++;
	    	rec = rec + 4;
	    }
	    ...
	    while (iterator.hasNext() && setProgress(i, iTopCount)){
			...
			final Object[] resultValues = new Object[3 + (dStressPeriod*4)];
