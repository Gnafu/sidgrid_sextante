 ...
 * Rudy Rossetto
 */
public class DrainPolylineToModelGridAlgorithm extends GeoAlgorithm{
...
//	         lines.addFilter(new BoundingBoxFilter(m_AnalysisExtent));
	         ...
	         final Class[] outputFieldTypes = new Class[2 + (spNumber*3)];
		     final String[] outputFieldNames = new String[2 + (spNumber*3)];
		     final Object[] resultValues = new Object[2 + (spNumber*3)];
		     ...
		     for (int n = 0; n < spNumber; n++){
		    	 ...
		    	 outputFieldNames[r] =  "eleva_"+ s;
		    	 outputFieldNames[r+1] =  "cond_"+ s;
		    	 outputFieldNames[r+2] =  "xyz_"+ s;
		    	 s++;
		    	 r = r+3;
	   ...
	   private void processLine(final Geometry geom,
	      ...
	      int elev_in = iTable.getFieldIndexByName("ELEV_I");
	      int elev_out = iTable.getFieldIndexByName("ELEV_E");
	      ...
	      for (int r=0; r<spNumber; r++)			          
	         {
	            ...
	            double s_elev_1i = (Double) timerecord.getValue(elev_in);	            
	            ...
	            resultValues[f] = s_elev_1i;  // eleva_
	            resultValues[f+1] = s_cond;   // cond_
	            resultValues[f+2] = m_xyz;    // xyz_
	            f = f+3;
	         }
	      recorditer.close();
	            ...
	            for (j = 0; j < iPoints; j++) {
	              ...
	              /*provo ad iserire la lettura degli stress period qua*/
	 		         ...
	 		         for (int r=0; r<spNumber; r++)			          
	 		         {
	 		            ...
	 		            double s_elev_1i = (Double) timerecord2.getValue(elev_in);
	 		            double s_elev_1e = (Double) timerecord2.getValue(elev_out);	 		            
	 		            double elevation = s_elev_1i - (((s_elev_1i - s_elev_1e)/(lenght/m_dDist))*k);	 		            
	 		            ...
	 		            resultValues[h] = elevation;
	 		            resultValues[h+1] = s_cond;
	 		            resultValues[h+2] = m_xyz;
	 		            h = h+3;
	 		         }
	 		       recorditer2.close();              
...
//	               resultValues[spNumber] = new Double(stage);
	   ...
	   public void defineCharacteristics() {
	      ...
	      setName(Sextante.getText("Lines to drain linear interpolator"));
...
//	      setUserCanDefineAnalysisExtent(true);
	      ...
	      try {
	         ...
	         m_Parameters.addNumericalValue(WIDTH, Sextante.getText("Width drain"),
	                  AdditionalInfoNumericalValue.NUMERICAL_VALUE_INTEGER, 1, 0, Integer.MAX_VALUE);
	         m_Parameters.addNumericalValue(XYZ, Sextante.getText("[xyz]"),
	                  AdditionalInfoNumericalValue.NUMERICAL_VALUE_INTEGER, 1, 0, Integer.MAX_VALUE);
	         m_Parameters.addTableField(LENGHT, Sextante.getText("Drain lenght"), LINES);
	         m_Parameters.addInputTable(TABLE, Sextante.getText("Sp Table"), true);
	         addOutputVectorLayer(RESULT, Sextante.getText("Drain_Points"), OutputVectorLayer.SHAPE_TYPE_POINT);
	      }
